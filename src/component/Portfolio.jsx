							...
							{port.projects?.map((proj, i) => (
								<div key={i} className=''>
									<div key={i} className='w-full  h-full relative group '>
										<img className='w-full h-[30rem] rounded-xl object-cover bg-cover group-hover:scale-y-115 big' src={proj.img} alt='project-cover-image' />
										<div className=' absolute bottom-[0] w-full h-full  group-hover:bg-gradient-to-br from-blue-400 to-amber-200/40 rounded-lg z-20 '>
											<div className=' flex items-start flex-col gap-3 justify-center px-3 absolute top-[15rem] left-[1rem] lg:left-[0] sm:left-[1rem]  xsm:left-[0.5rem] opacity-0 group-hover:opacity-100 z-40 text-center '>
												...
												<p className='font-semibold text-base max-w-[20rem]  leading-7 text-justify text-[#151533] '>{proj.text}</p>
												<a href={proj.url}>
													<button type='button' className=' border-2 p-2 cursor-pointer capitalize text-blue-900 text-xl hover:border-[#1d1e58] group-hover:text-[#e8ebf1] bg-[#30315c]'>
														{proj.btn}
													</button>
												</a>
											</div>
